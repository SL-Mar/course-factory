...
class NotionImporter:
    """Import a Notion workspace into Katja."""

    def __init__(self, token: str, store: PageStore, index: PageIndex) -> None:
        self.token = token
        self.store = store
        self.index = index
        self._id_map: dict[str, str] = {}  # notion_id → katja_id
        self.headers = {
    ...
    async def _import_page(self, client: httpx.AsyncClient, notion_page: dict, workspace: str) -> None:
        """Convert a Notion page to a Katja page."""
    ...
    async def _import_database(self, client: httpx.AsyncClient, db: dict, workspace: str) -> None:
        """Import a Notion database as a Katja page describing the schema."""
